 ...
 * Returns tasks that are dispatching but don't have an active agent assigned.
 * AVA polls this endpoint to auto-spawn agents for new tasks.
 */
export const GET: RequestHandler = async () => {
	const db = getDb();
	
	// Tasks in "dispatching" that haven't been picked up yet (no assignee)
	const pending = db.prepare(`
		...
		WHERE tasks.status = 'dispatching' AND (tasks.assignee IS NULL OR tasks.assignee = '')
